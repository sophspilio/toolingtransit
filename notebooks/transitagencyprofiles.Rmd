...
**Note** that this code includese the `GEOID`, which we will need to identify and connect data spatially. 
```{r, eval = F}
...
Oops, you'll want the census tracts to have the GEOID, so we'll reload that to include the right variables.
...
We'll also run this analysis at the route level. Unlike access to transit, the amount of service analysis is additive. So we'll run it at the route level and be able to add up all the service for each GEOID (Census Tract). 
