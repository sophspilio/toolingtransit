...
#joining agency stop data
#but RAIL will be joined later in the process!
...
### Acccess to Transit (for real this time)


Time to do some analysis! We will use `st_interpolate_aw` to find who has access to transit at the route level. The following function takes a route and creates a 1/4 mile (for bus), 1/3 mile (for Metrorail) , or 1 mile (for commuter rail) buffer at each stop in that route. Then we interpolate the demographic data to within that buffered area, to find how much of different populations have access to transit. 
...
#1/2 mile for heavy rail
access2transitHR <- function(route) {
  #pull just the stops within a 1/4 mile radius of county
  BuffJoin <- NovaStopsRoutes_2022 %>% filter(newroute_id == route) %>%
...
This function only find the interpolated demographic data for one route. To find this for each route, we use `lapply` to loop through every route in our file and run the function. 

Because of the difference in radii for commuter rail and bus, we run these separately and then combine them at the end. 

We are not including WMATA (Metrorail or Metrobus) in the access to transit analysis. So before we get started, we remove WMATA data from our `NovaStopsRoutes` file. Also, the interpolation doens't like non-numeric data, so we remove the GEOID from `CensusDataTracts`. 
...
So we looked at the route-level access to transit. We can also look at this at an agency level. To do that, we just alter the function a bit and run through each agency (except WMATA). 
...
We run this analysis at the route level first . Unlike access to transit, the amount of service analysis is additive. So we'll run it at the route level and be able to add up all the service for each GEOID (Census Tract). 
...
# Using Census Tracts
amount_serviceROUTE <- function(newroute_id){
...
Now we run it for the entire agency. We'll have to tweak the `amount_service` function, since we no longer need to iterate through each route. 
...
We can loop through a list of the agencies to make this easier, just like when we did it by routes. 

```{r, eval = F}
agencies <- NovaStopsRoutes_2022 %>% distinct(AgncyNm, Mode) %>% as.data.frame()


#run loop through all routes
nova_amountAGENCY <- lapply(agencies$AgncyNm, amount_serviceAGENCY) %>%
  do.call(rbind.data.frame, .)
```
